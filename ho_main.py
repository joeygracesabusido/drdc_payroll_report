...
def transaction():
    ...
    TransactionList = [
               ...
               {"Code": '2006',"Transaction":'MCC Computation'},
               {"Code": '2007',"Transaction":'Testing File'},
    ...
    if ans == '1000':
        ...
        return ExcellConnection.MCC_computation()
    
    elif ans == '2007':
        return ExcellConnection.test_reading_file2()
...
class ExcellConnection():
    ...
    def employee_list():
        ...
        df_sheet2 = pd.read_excel(file_path, sheet_name='Sheet4')
    ...
    def GCE_computation():
        ...
        total_sss_loan_return_add_df = pd.DataFrame({'DEPARTMENT': ['ADVANCE FROM PERSONNEL'], 'BOOKS': [total_sss_loan_return_add]})
        ...
        total_sss_total_loan_df = pd.DataFrame({'DEPARTMENT': ['SSS SALARY LOAN PAYABLE'], 'Total_Gross': [sss_loan_df]})
    ...
    def EP_computation():
        ...
        total_13th_month_less_df = pd.DataFrame({'DEPARTMENT': ['13th MONTH - LESS'], 'Total_Gross': [total_13th_month_less]})
        total_sss_remittance_df = pd.DataFrame({'DEPARTMENT': ['SSS/MEDICARE/ECC PAYABLE'], 'Total_Gross': [sss_total_payable]})
        total_sss_total_loan_df = pd.DataFrame({'DEPARTMENT': ['SSS LOAN PAYABLE'], 'Total_Gross': [sss_loan_df]})
        total_sss_total_calamitiy_df = pd.DataFrame({'DEPARTMENT': ['SSS CALAMITY LOAN PAYABLE'], 'Total_Gross': [sss_total_calamity]})
        total_phic_contri_payable_df = pd.DataFrame({'DEPARTMENT': ['PHILHEALTH CONTRIBUTIONS PAYABLE'], 'Total_Gross': [phic_contri_payable]})
        total_hdmf_contri_payable_df = pd.DataFrame({'DEPARTMENT': ['PAG-IBIG CONTRIBUTIONS PAYABLE'], 'Total_Gross': [hdmf_contri_payable]})
        total_hdmf_loan_df = pd.DataFrame({'DEPARTMENT': ['PAG-IBIG SALARY LOAN PAYABLE'], 'Total_Gross': [hdmf_loan_payable]})
    ...
    def MCC_computation():
        ...
        # Assuming 'BOOKS' is a column in the grouped DataFrame
        filtered_df = grouped_df[grouped_df['BOOKS'] == 'MCC']
    ...
    def test_reading_file():
        ...
        selected_book_key = input('Enter the number of your desired book: ')

        if selected_book_key in books:
            selected_book = books[selected_book_key]
            ...
            def find_similarity(row):
                if pd.notna(row['DEPARTMENT']):
                    ...
                    if fuzz.partial_ratio(str(row['Name']), str(best_match)) > 80:
                        if 'DEPARTMENT' in payroll_comp.columns:
                            matches = payroll_comp.loc[(payroll_comp['Name'] == best_match) & (payroll_comp['BOOKS'] == selected_book), 'DEPARTMENT']
                            if not matches.empty:
                                return matches.values[0]
                        return 'Not Found'
            ...
            # Filter the dataframe to contain only rows with the selected book
            selected_df = merged_df[merged_df['BOOKS'] == selected_book]

            print(selected_df)

            transaction()
        else:
            print("Invalid selection. Please select a valid book.")


    @staticmethod
    def test_reading_file2():
        books = {
            '1': 'GENERAL COMMON EXPENSE',
            '2': 'ELISTON PLACE',
            '3': 'WELLINGTON PLACE',
        ...
        if selected_book_key in books:
            ...
            # Assuming 'BOOKS' is a column in the grouped DataFrame
            filtered_df = grouped_df[grouped_df['BOOKS'] == selected_book]
